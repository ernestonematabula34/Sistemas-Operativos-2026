...
sthread_t sthread_create(sthread_start_func_t start_routine, void *arg, int priority) {
  ...
  IMPL_CHOOSE(newth = sthread_pthread_create(start_routine, arg),
              newth = sthread_user_create_with_priority(start_routine, arg, priority));
  return newth;
}
...
int sthread_nice(int nice) {
  return IMPL_CHOOSE(0, sthread_user_nice(nice));
}

sthread_t sthread_create_with_priority(sthread_start_func_t start_routine,
